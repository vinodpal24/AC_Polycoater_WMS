...
import com.example.acPolycoaters.Global_Classes.MyApp
...
object NetworkClients {
    ...
    private fun provideLoggingInterceptor(): HttpLoggingInterceptor {
        return HttpLoggingInterceptor { message ->
            ...
            while (i < message.length) {
                ...
                Log.d("HTTP_LOG", message.substring(i, end))
                i += maxLogSize
            }
        }.apply {
    ...
    private fun provideSessionOkHttpClient(context: Context): OkHttpClient {
        ...
        return OkHttpClient.Builder()
            ...
            .addInterceptor { chain ->
                ...
                if (sessionManagement.getSessionTimeout(context).isNullOrBlank() ||
                    response.code == 301 || response.code == 401
                ) {
    ...
    fun create(context: Context): NetworkInterface {
        sessionManagement = SessionManagement(context)
        val isLogin = sessionManagement.getFromWhere(context) == "Login"
        val client = if (isLogin) provideDefaultOkHttpClient() else provideSessionOkHttpClient(context)
    ...
    /*private fun createSessionInterceptor(context: Context, sessionManagement: SessionManagement): Interceptor {
        return Interceptor { chain ->
            val original = chain.request()
            val requestBuilder = original.newBuilder()
                .addHeader("Cookie", "B1SESSION=${sessionManagement.getSessionId(context)}")
            ...
            // Handle session timeout or invalidation
            if (sessionManagement.getSessionTimeout(context).isNullOrEmpty() || response.code == 301 || response.code == 401) {
                logoutScreen(context)
            }
