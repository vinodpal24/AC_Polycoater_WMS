package com.example.acPolycoaters.ui.deliveryOrderModule.UI
...
import com.example.acPolycoaters.databinding.ActivityDeliveryDocumentLineBinding
import com.example.acPolycoaters.ui.deliveryOrderModule.Adapter.DocumentOrderLineAdapter
...
class DeliveryDocumentLineActivity : AppCompatActivity(), DocumentOrderLineAdapter.AdapterCallback {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setAdapter()
        if (!deliveryValueList.isNullOrEmpty()) {
            if (Prefs.getString(AppConstants.BPLID, "").isNotEmpty()) {
                getDocSeriesApi(Prefs.getString(AppConstants.BPLID, ""))
            } else {
                GlobalMethods.showError(this@DeliveryDocumentLineActivity, "Please select branch from setting screen.")
            }
    ...
    private fun getDocSeriesApi(bplidNum: String) {
        if (networkConnection.getConnectivityStatusBoolean(applicationContext)) {
            try {
                ...
                val customQtyClient = QuantityNetworkClient.create(this@DeliveryDocumentLineActivity)
                customQtyClient.getDocSeries(bplidNum, "15").apply {
